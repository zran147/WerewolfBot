...
async def peringatan(channel, total, ketika_sisa):
    global speed
    await asyncio.sleep(speed*(total - ketika_sisa))
    embed = discord.Embed(
    ...
    while True:
        ...
        if reaction.emoji in emojis:
            ...
            for r in cache_msg.reactions:
                if user in [user async for user in r.users()] and str(r) != str(reaction.emoji):
...
@bot.command()
async def play(ctx, kecepatan='1', arg=''):
    global state, speed
...
async def start(ctx):
    ...
    if state == 'ready to play':
        state = 'playing'
        global speed, channel, players, emoji_to_player, users, isGameEnded, emoji, killed, roles, seer, werewolf, guard
...
async def diskusi(channel, hari, role, dead_role, hasil):
    global players, emoji_to_player, speed
    ...
    #await asyncio.sleep(speed*10)
        ...
        asyncio.create_task(voting(poll)),
        asyncio.create_task(voting(poll)),
        asyncio.create_task(peringatan(channel, 30, 10))
    ], timeout=30)
